...
struct
    ...
    let rotate_l_stmt dst sz count =
      let high = BinOp (Shr, Lval dst, BinOp (Sub, count, Const (Word.of_int (Z.of_int (sz-1)) sz))) in
    ...
    let rotate_r_stmt dst sz count =
      ...
      let low = BinOp (Shr, Lval dst, BinOp(Sub, count, Const (Word.of_int (Z.of_int (sz-1)) sz))) in
      let src = BinOp(Add, high, low) in
