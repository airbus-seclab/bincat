  ...
  module Make(Domain: Domain.T) =
	...
	struct
	  ...
	  type t = {
	      ...
	      mutable v: Domain.t option; 		  (** abstract value ; None means that not set yet *)
	      mutable ctx: ctx ; 		  (** context of decoding *)
	...
	struct
      ...
      (* (\** dummy state *\) *)
      (* let dummy_state ip = {id = 0; ip = ip; v = None; stmts = []; ctx = {op_sz = Domain.Asm.Word.default_size(); addr_sz = Domain.Asm.Address.default_size()}; internal = false} *)
...
(*   	  | Some v, None -> Some v *)
(* 	  | Some v, Some v' -> Some (Domain.join v v') *)
(* 	in *)
(* 	let rec find succs = *)
(* 	  match succs with *)
(* 	    s::succs' ->  *)
...
(* 	with Not_found -> add (), true *)
      ...
      (* 	  true *)
      (* 	| Some v ->  *)
      (* 	   let v2  = Domain.join v' v in *)
      (* 	   s.v <- Some v2; *)
      (* 	   Domain.contains v' v2 *)
