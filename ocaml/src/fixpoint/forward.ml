        ...
        let t =
            List.fold_left (fun t v ->             
                ...
                let a =
                  ...
                  | _l  -> raise (Exceptions.Analysis (Exceptions.Too_many_concrete_elements "multiple return addresses"))
                in
      ...
      and process_vertices (vertices: Cfa.State.t list) (s: Asm.stmt) : (Cfa.State.t list * Taint.Set.t) * bool =
        ...
        | Jmp_exn ->
             ...
             | Call (A a) ->
                add_to_fun_stack a;
                let vertices' =
                  List.map (fun v -> { v with Cfa.State.v = D.call v.Cfa.State.v}) vertices
                in
                begin
                  try
