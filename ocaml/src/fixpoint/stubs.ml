...
struct
    ...
    let heap_deallocator (ip: Data.Address.t) _ (d: domain_t) _ret args _prev: domain_t * Taint.Set.t =
    ...
    let strlen (_ip: Data.Address.t) _ (d: domain_t) ret args _prev: domain_t * Taint.Set.t =
    ...
    let memcpy (_ip: Data.Address.t) _ (d: domain_t) ret args _prev: domain_t * Taint.Set.t =
    ...
    let memset (_ip: Data.Address.t) _ (d: domain_t) ret args _prev: domain_t * Taint.Set.t =
      let arg0 = args 0 in
    ...
    let sprintf (_ip: Data.Address.t) _ (d: domain_t) ret args _prev: domain_t * Taint.Set.t =
      let dst = Asm.Lval (args 0) in
      let format_addr = Asm.Lval (args 1) in
      let  va_args = shift args 2 in
      print d ret format_addr va_args (Some dst)

    let sprintf_chk (_ip: Data.Address.t) _ (d: domain_t) ret args _prev: domain_t * Taint.Set.t =
      let dst = Asm.Lval (args 0) in
      let format_addr = Asm.Lval (args 3) in
      let va_args = shift args 4 in
      print d ret format_addr va_args (Some dst)

    let printf (_ip: Data.Address.t) _ d ret args _prev =
    ...
    let printf_chk (ip: Data.Address.t) calling_ip d ret args prev = printf ip calling_ip d ret (shift args 1) prev

    let puts (_ip: Data.Address.t) _ d ret args _prev =
    ...
    let putchar (_ip) _ d ret args _prev =
      let str = Asm.Lval (args 0) in
    ...
    let bin_exit (_ip) _ _d _ret _args _prev =
      raise (Exceptions.Stop "on exit call")
      
    let stubs = Hashtbl.create 5

    let process ip calling_ip d fun_name call_conv prev: domain_t * Taint.Set.t * Asm.stmt list =
      let apply_f, arg_nb =
        try Hashtbl.find stubs fun_name
        with Not_found -> L.abort (fun p -> p "No stub available for function [%s]" fun_name)
      in
      let d', taint =
        try apply_f ip calling_ip d call_conv.Asm.return call_conv.Asm.arguments prev
        with
