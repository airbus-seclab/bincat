let typing_rule_stmts name callconv =
  if Hashtbl.mem Types.typing_rules name then
    begin
      let rule = Hashtbl.find Types.typing_rules name in
      let prologue =
        match fst rule with
        | None -> []
        | Some args ->
           List.mapi (fun i (typ,_name) -> Asm.Directive (Asm.Type (callconv.Asm.arguments i, typ))) args in
      let epilogue =  [ Asm.Directive (Asm.Type (callconv.Asm.return, snd rule)) ] in
      prologue, epilogue
    end
  else [], []
