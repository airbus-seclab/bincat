...
(** Vertex abstract data type *)
	...
	op_sz  : int
      ...
      type t = {
	id	     : int; 
	mutable s    : State.t option; 
	mutable stmts: Asm.stmt list;     (* None for the state field when the content has never been set *)
	mutable ctx  : ctx; 
	a	     : Data.Address.t;
	internal : bool 
      } 
  
      let vertex_cpt 	   = ref 0
      let new_vertex_id () = vertex_cpt := !vertex_cpt + 1; !vertex_cpt

      let equal b1 b2   = b1.id = b2.id	
      let compare b1 b2 = b1.id - b2.id
      let hash b 	= b.id
    end
(** *)
...
(** *)    
    ...
    module G = Graph.Imperative.Digraph.ConcreteBidirectionalLabeled(Vertex)(Label)
    open Vertex
...
(** dummy vertex *)
    let dummy_vertex a = {id = 0; a = a; s = None; stmts = []; ctx = {op_sz = Data.Word.default_size(); addr_sz = Data.Address.default_size()}; internal = false}
      
(** returns true whenever the two given contexts are equal *)
    let ctx_equal c1 c2 = c1.addr_sz = c2.addr_sz && c1.op_sz = c2.op_sz 

(** [add_vertex g pred a s stmts ctx] creates a new vertex _v_ in _g_ with 
    - a as address;
    - stmts as list of statements;
    - s as state (if already in the CFG ; then previous value is widened with s)
    - prev.s as state;
    - ctx as decoding context *)
    let add_vertex g pred a s stmts ctx i = 
      let add () = 
	let v = { 
	  id 	= new_vertex_id(); 
	  s 	= s; 
	...
	v
      in
      let update prev news =
	match prev, news with
	  None, _ -> news
	| Some s, None -> Some s
	| Some s, Some s' -> Some (State.widen s s')
      in
      let rec find succs =
	match succs with
	  v::succs' -> 
	    if Data.Address.compare v.a a = 0 && ctx_equal v.ctx ctx && v.internal = i then 
	      begin
		v.s <- update v.s s;
		v
	      end
...
(** updates the state field of the given vertex *)
  let update_state v s'= 
    match v.s with
	None   -> 
	  v.s     <- Some s';
	  true
      | Some s -> 
	let s2  = State.widen s s' in
	v.s <- Some s2;
	State.contains s' s2

(* updates the context and statement fields of the given vertex *)
  let update_stmts v stmts op_sz addr_sz = 
    v.stmts <- stmts; 
    v.ctx   <- {addr_sz = addr_sz; op_sz = op_sz}
...
let make_rep s str_stmt regs i =
  ...
  in
  let incr_blk, _ = Cfg.add_vertex s.g instr_blk s.a instr_blk.Cfg.Vertex.s incr ctx true                                                                                                                              in
