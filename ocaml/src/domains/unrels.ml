    ...
    let mem_to_addresses m e check_address_validity =
      ...
      | Val m' ->
         USet.fold (fun u (addrs, t) ->
             ...
             with Exceptions.Analysis a   ->
               L.analysis (fun p -> p "analysis stops for that context: %s" (Exceptions.string_of_analysis a));
               addrs, t) m' (Data.Address.Set.empty, Taint.Set.singleton Taint.U)
      ...
      | Val m' ->
         let res =
           USet.fold (fun u o ->
               ...
               | Some o ->
                  if o = o' then Some o
                  else raise (Exceptions.Analysis (Exceptions.Empty "Unrels.get_offset_from: different offsets found"))) m' None
         in
         match res with
         | Some o -> o
