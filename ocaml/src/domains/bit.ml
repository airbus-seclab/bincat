...
let meet b1 b2 =
  ...
  | ZERO, ZERO            -> ZERO
  | ONE, ONE              -> ONE
  | ONE, ZERO | ZERO, ONE -> raise (Exceptions.Analysis (Exceptions.Empty "bit.meet"))
  | b, TOP | TOP, b       -> b
...
let to_z v =
  match v with
  | TOP  -> raise (Exceptions.Analysis (Exceptions.Too_many_concrete_elements "bit.to_z: imprecise bit value"))
  | ZERO -> Z.zero
  | ONE  -> Z.one
