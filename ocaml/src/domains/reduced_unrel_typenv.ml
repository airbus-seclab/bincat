...
(struct
  ...
  let type_of_exp tenv uenv e =
    ...
    | Asm.Lval (Asm.M (e, _sz)) ->
	 ...
	 try
	   ...
	   | _ -> Types.UNKNOWN
	 with Exceptions.Too_many_concrete_elements -> Types.UNKNOWN
       end
    | _ -> Types.UNKNOWN
  ...
  let set_type (lv: Asm.lval) (typ: Types.t) ((uenv, tenv): t): t =
   ...
   let tenv' =
	...
	with Exceptions.Too_many_concrete_elements -> T.forget tenv
   in
   uenv, tenv'
  ...
  let char_type uenv tenv dst =
     ...
     with Exceptions.Too_many_concrete_elements -> T.top
