...
(struct
  ...
  let set_lval_to_addr (lv: Asm.lval) (addrs: (Data.Address.t * Log.History.t) list) ((uenv, tenv, henv, senv): t): t*Taint.Set.t =
    ...
    try
      ...
      let ptr_typ =
        match buf_typ with
        | Types.C t -> Types.C (TypedC.Ptr t)
  ...
  let char_type uenv tenv henv senv dst =
     let typ = Types.C (TypedC.Int (Newspeak.Signed, 8)) in
     try
