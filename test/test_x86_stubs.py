...
def test_sprintf_num(tmpdir, val, fmt, mod, zeropad, sz):
    ...
    assert expected == bc.result.last_node.unrels['0'].get_string("", 0x100), (repr(fmtstr)+"\n"+bc.listing)
    assert len(expected) == bc.result.last_reg("eax").value, (repr(fmtstr)+"\n"+bc.listing)
...
def test_sprintf_string(tmpdir, val, numl, numr):
    ...
    assert expected == bc.result.last_node['0'].get_string("", 0x100), (repr(fmtstr)+"\n"+bc.listing)
    assert len(expected) == bc.result.last_reg("eax").value, (repr(fmtstr)+"\n"+bc.listing)
...
def test_memcpy_call(tmpdir, src):
    ...
    assert src == bc.result.last_node['0'].get_string("", 0x10000), (repr(src)[:20]+"\n"+bc.listing)
